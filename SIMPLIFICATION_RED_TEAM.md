...
| Removing `Klyuchnik` facade removes secret isolation | Use exact secret route, alter one route dimension, request reveal | Rejected: `SecretBroker` accepts only matching Propusk; reveal always denies |
...
| Removing duplicate checks widens authority | Try raw action, wrong scope, unknown capability, secret destination, protected export | Rejected by Propusk, Krosna, Ruslo, Klyuchnik, and executor boundaries |
...
- Targeted Gnezdo/Klyuchnik mutation run passed with 46/55 mutants caught and no
