...
| Diode | Protected data is exported or a direction is confused | READ, EXPORT, and TRANSFER are separate directed decisions | `Diode::evaluate` plus Gateway final flow gate | Protected-derived output can cross an allowed-looking public route | Required enforcement |
...
| Pechat broker | Model sees or reveals raw credential material | Secrets stay broker-side; use requires exact route and reveal is denied | `SecretHandle`, `SecretBroker`, exact `secret.use` Krosna route | Raw secret disclosure or uncontrolled secret use | Required enforcement |
...
| Niti | Derived output loses source lineage | Every supported derivation retains bounded parent provenance | `Provenance::derived_from`, `Niti`, `TaggedData` | Diode and diagnostics can no longer distinguish protected origin | Supporting security state |
...
The minimum physical enforcement core is Krosna + Propusk + Diode + Zaslon +
Pechat's broker boundary. Gnezdo, Niti, and Metka are not redundant labels:
