...
block-art баннером. Он встроен прямо в `tkach-cli`: runtime не читает asset-файл
и ничего не скачивает. Баннер занимает 113 колонок и 11 строк; `TKACH_BANNER=compact`
его скрывает, а `TKACH_BANNER=png` включает присланный PNG как цветные пиксельные
ячейки. В слишком маленьком окне используется компактный заголовок. Этот путь
