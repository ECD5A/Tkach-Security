...
**Стрелки вверх/вниз** (или **1–6**) выбирают пункт, **Enter** открывает действие,
...
готовность, запустить офлайн-демо и разобраться с подключением. Оно не
...
Интерактивное меню — компактная кроссплатформенная панель Ratatui с выбранным
действием, локальным статусом и подсказками управления. `NO_COLOR=1` отключает
цвет выделения; CLI остаётся рабочим в терминалах без цвета. CLI управляет
раскладкой и цветами, а настоящий шрифт задаётся эмулятором терминала.
