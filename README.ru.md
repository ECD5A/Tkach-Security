...
действием, локальным статусом, подсказками управления и полным баннером Tkach в
просторном терминале (пресет 132 x 40 показывает его без обрезки). Автовыбор
использует встроенный ASCII-вариант в старых консолях и подготовленный
Unicode-арт в совместимых терминалах: шрифт при запуске не скачивается. Режим
можно задать через `TKACH_BANNER=unicode|ascii|compact`; в меньшем окне остаётся
компактная шапка, минимальный размер — 44 x 18. `NO_COLOR=1` отключает цвет
выделения. CLI управляет раскладкой и цветами, а настоящий шрифт и его размер
задаются эмулятором терминала.
