...
просторном терминале (пресет 132 x 40 показывает его без обрезки). В меньшем
окне остаётся компактная шапка; минимальный размер — 44 x 18. `NO_COLOR=1`
отключает цвет выделения. CLI управляет раскладкой и цветами, а настоящий шрифт
и его размер задаются эмулятором терминала.
