...
просторном Linux/WSL-терминале (пресет 132 x 40 показывает его без обрезки).
В native Windows намеренно остаётся стабильная компактная шапка, потому что
поддержка шрифтов зависит от конкретной консоли. В Linux/WSL баннер можно
скрыть через `TKACH_BANNER=compact`; в меньшем окне он также заменяется
компактной шапкой. `NO_COLOR=1` отключает цвет выделения. CLI управляет
раскладкой и цветами, а настоящий шрифт и его размер задаются эмулятором
терминала.
