...
the thin productization CLI, HTTP adapter, Rust client, and MCP stdio adapter.
...
The workspace contains seven Rust packages:
...
- tkach-mcp: a separate bounded MCP stdio adapter over tkach-client, exposing
  one strict tkach_run tool and no direct Core or executor surface.
...
No deprecated primitive name, Streamable HTTP transport, public internet
gateway, UI, cloud control plane, or multi-language SDK is implemented at this
baseline. The MCP adapter and Rust client are local-only, and the CLI is local
onboarding only; none is a general-purpose agent runner.
...
- 113 core, 9 composition, 9 independent-oracle, 49 Gateway unit, 25 Gateway
  ...
  5 MCP adapter, 14 product-proof, 13 real-effect, 34 provider, and 1 opt-in
  live
...
the next boundary is distribution artifacts. No speculative provider, UI,
cloud, or public gateway implementation is implied by these adapters.
